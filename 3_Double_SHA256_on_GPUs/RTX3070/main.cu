...
__global__ void sha256_cuda(JOB ** jobs, uint32_t n, uint32_t j, OUT * outs) {
  ...
  for (uint32_t i = index; i < n; i += stride){
		...
		//printf("Nonce = %d\n",j*n+i);
		//sha256_update_1(&ctx, jobs[i]->data, jobs[i]->digest);
		sha256_transform_2(&ctx, jobs[i]->data1, temp1);
		sha256_transform_1(&ctx, temp1, jobs[i]->data2, temp2);
		//sha256_transform_1(&ctx, jobs[i]->H, jobs[i]->data, jobs[i]->digest);
		...
		//sha256_transform_1(&ctx, jobs[i]->data, buf2);
...
int main(void)
{
	...
	for (i=0;i<8;i++){
		for(j=i*8;j<i*8+8;j++){
				if(Prev_Hash[j]>='0' && Prev_Hash[j]<='9'){
					Digit=Prev_Hash[j]-0x30;
				}
				else if((Prev_Hash[j]>='A' && Prev_Hash[j]<='F') || (Prev_Hash[j]>='a' && Prev_Hash[j]<='f')){
					switch(Prev_Hash[j])   {
				...
				IntValue+= Digit*pow(16,Prev_Hash_Counter);
			Prev_Hash_Counter--;
		}
		Prev_Hash_LitEndian[7-i] = LitToBigEndian(IntValue);
		Prev_Hash_Counter=7;
		IntValue = 0;
	}
	...
	uint32_t  Merk_Hash_Counter = 7;
	uint32_t  Merk_Hash_LitEndian[8];
	
	for (i=0;i<8;i++){
		for(j=i*8;j<i*8+8;j++){
				if(Merk_Hash[j]>='0' && Merk_Hash[j]<='9'){
					Digit=Merk_Hash[j]-0x30;
				}
				else if((Merk_Hash[j]>='A' && Merk_Hash[j]<='F') || (Merk_Hash[j]>='a' && Merk_Hash[j]<='f')){
					switch(Merk_Hash[j])   {
						case 'A': case 'a': Digit=10; break;
						case 'B': case 'b': Digit=11; break;
						case 'C': case 'c': Digit=12; break;
				...
				IntValue+= Digit*pow(16,Merk_Hash_Counter);
			Merk_Hash_Counter--;
		}
		Merk_Hash_LitEndian[7-i] = LitToBigEndian(IntValue);
		Merk_Hash_Counter=7;
		IntValue = 0;	
	}
